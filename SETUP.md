...
Get onexus running on a fresh computer (Windows, macOS or Linux). "Gems" are
...
```bash
git clone <this-repo-url> ~/onexus-repo
cp -r ~/onexus-repo/skills/onexus ~/.claude/skills/     # Claude Code
```

Other master harnesses (optional — the skill is harness-agnostic):

```bash
cp -r ~/onexus-repo/skills/onexus ~/.config/opencode/skills/   # opencode
# codex/gemini/etc.: your harness's equivalent skills folder
```

Updating later: `git -C ~/onexus-repo pull`, then repeat the `cp`.
...
Test: `python3 ~/.claude/skills/onexus/scripts/okf_validate.py --help`
...
Create `~/.onexus/executors.json` (upgrading from okfbrain? your old
`~/.okfbrain/executors.json` is still read — `~/.onexus` wins on conflict):
...
python3 ~/.claude/skills/onexus/scripts/okf_embed.py <a-Gem>   # builds _index/ inside the Gem
```
...
- **git** (recommended): version each Gem in a private repo (the skill offers
  this at creation time) and clone it anywhere;
- **cloud drive**: drag the folder;
- **scp/rsync** between machines.

Each Gem's `viz.html` opens in any browser (internet needed only on first
load, for the CDN libraries).

## 5. Final smoke test (2 minutes)

```bash
cd ~/.claude/skills/onexus
python3 scripts/okf_selftest.py                               # 24 checks — ALL GREEN?
python3 scripts/okf_loop.py --list-executors                  # fleet visible?
python3 scripts/okf_validate.py <a-Gem>                     # 0 errors?
python3 scripts/okf_status.py   <a-Gem>                     # X-ray ok?
python3 scripts/okf_search.py   <a-Gem> "some term"         # search ok?
```

A new executor NEVER goes to production without the **admission test**
(`reference/EXECUTORS.md`, "Checks" section): 1–2 cycles on a scratch Gem +
