...
# Windows: force UTF-8 stdout (cp1252 console breaks on ✓/✗ and non-Latin scripts)
try:
...
def check_concept(path: Path, rel: str, report: Report) -> None:
    ...
    try:
        ...
        report.err(rel, f"§9.1 not readable as UTF-8: {exc}".replace(chr(10), " "))
        return
    ...
    # `_`-meta files (e.g. _loop-state.md, _learning/…) are working state, not
    # knowledge: hard rules still apply, but recommended-field nagging is noise.
...
def validate(bundle: Path) -> Report:
    report = Report()
    # `_staging/` is the fan-out miners' transient work area (drafts + raw
    # sources, pre-knowledge) — never part of the bundle proper.
    md_files = sorted(p for p in bundle.rglob("*.md")
                      if p.is_file()
                      and not p.relative_to(bundle).as_posix().startswith("_staging/"))
    for path in md_files:
        rel = path.relative_to(bundle).as_posix()
        name = path.name
        if name == "index.md":
